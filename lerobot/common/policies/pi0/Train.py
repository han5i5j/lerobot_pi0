...
def evaluate(acc,
    ...
    if policy.use_ema:
        ...
        policy.net.eval()
    with torch.no_grad():
        for epoch in tqdm(range(num_evaluate_epochs), desc=f"evaluate epochs", disable=not acc.is_main_process):
            batch_metric = {
                'total_loss': 0,
                # 'grad_norm_before_clip': 0,
                'dataload_time': 0,
            ...
            while batch is not None:
                with acc.accumulate(policy.net):
                    batch = preprocessor.process(batch, train=False)
                    pred = policy.infer(batch)
                    real_pred = preprocessor.back_process(pred)
                    loss = {'total_loss': 0}
                    for key in policy.loss_configs:
                        loss_func = policy.loss_configs[key]['loss_func']
                        weight = policy.loss_configs[key]['weight']
                ...
                if batch_idx % print_interval == 0 and batch_idx != 0:
                    avg_metric['dataload_percent_first_gpu'] = avg_metric['dataload_time'] * print_interval / (
                                time() - clock)
                    avg_metric['fps_first_gpu'] = (bs_per_gpu * print_interval) / (time() - clock)
                    clock = time()
